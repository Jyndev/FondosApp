...
# Función para cambiar el fondo de pantalla con `awww` y la transición personalizada
wallpaper() {
    if [[ "$1" =~ \.(png|jpg|jpeg|gif)$ ]]; then
        if [ "$METODO_FONDO" == "video" ]; then
            pkill mpvpaper # Detener el proceso de video
            awww-daemon &   # Iniciar el daemon de awww
        fi
        # Si es una imagen o GIF, usar awww para cambiar el fondo con transición
        awww img "$1" --transition-type "outer" --transition-pos "0.854,0.977" --transition-step 90
     ...
     if [ "$METODO_FONDO" == "wallpaper" ]; then
            pkill awww-daemon # Detener el daemon de awww
        fi
        ...
        awww img "$HOME/.cache/liveWallpaper/wall-video.jpg" --transition-type "outer" --transition-pos "0.854,0.977" --transition-step 90
